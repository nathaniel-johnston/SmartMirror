...
# Nathaniel's defined methods
def audioIsSpotifySearch(transcript):
    ...
    # not a spotify command
    if transcript.find("spotify") == -1:
        return "", ""
    
    if transcript.find("album") != -1:
        itemType = "album"
        # play the album x by y on spotify
        tempQuery = transcript[transcript.find("album ")+len("album "):transcript.find("on ")]
    elif transcript.find("artist") != -1:
        itemType = "artist"
        # play the album x by y on spotify
        tempQuery = transcript[transcript.find("artist ")+len("artist "):transcript.find("on ")]
    elif transcript.find("playlist") != -1:
        itemType = "playlist"
        # play the playlist x on spotify
        tempQuery = transcript[transcript.find("playlist ")+len("playlist "):transcript.find("on ")]
    elif transcript.find("song") != -1:
        itemType = "track"
        # play the song x by y on spotify
        tempQuery = transcript[transcript.find("song ")+len("song "):transcript.find("on ")]
    else:
        itemType = "track,album,playlist,artist"
        # Play x by y on spotify
        tempQuery = transcript[len("play "):transcript.find("on ")]
    
    # remove spaces and 'by'
    i = 0
    while(i < len(tempQuery)):
        if tempQuery[i] == " ":
            query += "+"
        elif tempQuery[i] == "b" and tempQuery[i+1] == "y":
            i += 2
        else:
            query += tempQuery[i]
        i += 1
...
class SampleAssistant(object):
    ...
    # end of Nathaniel defined functions
