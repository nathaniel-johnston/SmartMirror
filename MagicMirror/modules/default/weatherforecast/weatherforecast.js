...
Module.register("weatherforecast", {
	...
	getParams: function () {
		var params = "?";
		if (this.config.geo) {
			params += "lat=" + this.config.geo.lat + "&lon=" + this.config.geo.lon;
		} else {
			this.hide(this.config.animationSpeed, { lockString: this.identifier });
			return;
		}

		if(this.config.toExclude != null) {
			params += "&exclude=" + this.config.toExclude;
		}
	...
	processWeather: function (data) {
		...
		for (var i = 0, count = data.daily.length; i < count; i++) {
			...
			forecastData = {
				...
				rain: this.processRain(forecast, data.daily)
			};
